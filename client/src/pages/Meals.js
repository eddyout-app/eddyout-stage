...
export default function Meals() {
    ...
    function getMealDates(startDate, endDate) {
        const dates = [];
    ...
    return (<div className="bg-light-neutral min-h-screen py-10 px-4 font-body text-textBody">
      ...
      <div className="overflow-y-auto max-h-[80vh] pr-2 mx-auto">
        {" "}
        {tripDates.map((date, i) => (<div key={date.toISOString()} className="mb-6">
            ...
            {meals[i]?.map((meal, j) => (<DailyMeals key={`${date.toISOString()}-${meal.mealType}-${j}`} date={date} index={i + 1} endDate={trip.endDate} meal={meal} onMealChange={(field, value) => {
                    const updated = [...meals];
                    updated[i][j] = {
                        ...meals[i][j],
                        [field]: value,
