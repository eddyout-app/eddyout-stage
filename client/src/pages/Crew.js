...
export default function Crew() {
    ...
    console.log(user);
    if (!user) {
        return (<div className="text-center mt-10 text-textBody font-body text-lg">
        Loading...
      </div>);
    }
    return (<div className="bg-light-neutral min-h-screen py-10 px-4 font-body text-textBody">
      <h1 className="text-4xl font-header text-primary mb-6 text-center">
        Trip Crew
      </h1>

      {crew.length === 0 ? (<p className="text-center">No crew assigned to this trip yet.</p>) : (<ul className="space-y-2 max-w-3xl mx-auto">
          {crew.map((member) => (<li key={member.user.id} className="bg-white rounded-md shadow-sm px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 border border-gray-200">
              ...
              <button onClick={() => handleDelete(member.id)} className="text-red-600 hover:text-red-800 px-3 py-1 rounded-md text-sm transition duration-150">
                🗑️
              </button>
            </li>))}
        </ul>)}
      <div className="mt-8 text-center">
        ...
        <select value={selectedUserId || ""} onChange={(e) => setSelectedUserId(e.target.value)} className="border border-gray-300 rounded px-4 py-2">
        ...
        <button onClick={handleJoinCrew} className="btn-dark ml-4 bg-primary text-white px-6 py-2 rounded shadow hover:bg-primary-dark">
          Add to Crew
        </button>
      </div>
    </div>);
