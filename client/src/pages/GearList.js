...
export default function GearList() {
    ...
    return (<div className="bg-light-neutral min-h-screen py-10 px-4 font-body text-textBody">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {showForm && (<div className="mb-6 space-y-2">
            <input type="text" value={newItemName} onChange={(e) => setNewItemName(e.target.value)} placeholder="Enter gear item name" className="w-full border border-gray-300 rounded px-3 py-2"/>
            <button className="btn-dark" onClick={handleAddGear}>
              Submit
            </button>
          </div>)}

        {loading ? (<p className="text-center">Loading gear items...</p>) : gearItems.length === 0 ? (<p className="text-center">No gear items found for this trip.</p>) : (<ul className="space-y-4">
            {gearItems.map((item) => (<GearItem key={item.id} item={item} userId={userId} onClaimToggle={handleClaimToggle}/>))}
          </ul>)}
      </div>
    </div>);
}
