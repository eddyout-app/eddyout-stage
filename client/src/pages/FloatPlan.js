...
export default function FloatPlan() {
    ...
    return (<div className="bg-light-neutral min-h-screen py-10 px-4 font-body text-textBody">
      ...
      <div className="floatplan-content">
        {tripDates.map((date, i) => (<ScheduleDay key={date.toISOString()} date={date} index={i + 1} endDate={trip.endDate} location={locations[i]?.location || ""} isSaved={locations[i]?.isSaved} onLocationChange={(newLoc) => {
